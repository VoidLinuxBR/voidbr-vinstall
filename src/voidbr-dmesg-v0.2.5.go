...
func monitorFile(path string) {
	...
	// 2. Modo Tail (usando reader manual para evitar problemas de buffer)
	// Usamos um buffer simples para não depender de pacotes extras
	for {
		// Simula um tail com leitura contínua
		fi, _ := file.Stat()
