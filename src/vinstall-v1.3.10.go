/*
    ...
    Updated:   ter 23 jun 2026 17:15:00 -04
    Version:   1.3.10
...
// --- NOVO MOTOR DE BUSCA (DETALHADO COM SUPORTE A REGEX) ---
	...
	for dirName, repoURL := range repoMap {
		...
		go func(p, url string) {
			...
			for _, pkgData := range index {
				...
				match := false
				if isRegex {
					match = re.MatchString(pkgVer)
				} else {
					match = strings.Contains(strings.ToLower(pkgVer), strings.ToLower(query))
				}
...
func printUsage() {
	...
	fmt.Printf("  %-20s %s\n", green("-Ss <query>"), white("Busca termo nos repositórios remotos (Aceita Regex, entre aspas)"))
	fmt.Println("\nManutenção:")
