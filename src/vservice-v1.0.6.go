/*
    ...
    Updated:   qua 24 jun 2026 12:35:00 -04
    Version:   1.0.6
...
// fzfSelect agora filtra baseada na ação
func fzfSelect(action string) string {
	...
	for _, f := range files {
		...
		isActive := false
		if _, err := os.Lstat(filepath.Join(ActiveDir, name)); err == nil {
			isActive = true
		}

		if (action == "add" && !isActive) || (action == "remove" && isActive) {
...
func doAdd(s string) {
	...
	if _, e := os.Lstat(active); e == nil {
		warn("serviço '" + s + "' já está adicionado/habilitado")
		return
	}
...
func doRemove(s string) {
	...
	if _, e := os.Lstat(active); os.IsNotExist(e) {
		warn("serviço '" + s + "' não está habilitado (nada a remover)")
		return
	}
...
func main() {
	...
	if len(os.Args) < 3 {
		// Ajuste de filtro para o menu interativo
		fzfAction := "other"
		if action == "add" || action == "enable" || action == "install" {
			fzfAction = "add"
