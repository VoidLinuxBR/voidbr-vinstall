/*
    ...
    Updated:   qui 05 fev 2026 02:45:00 -04
            ...
            if strings.Contains(data, file) {
                ...
                for _, line := range lines {
                    if strings.Contains(line, "<string>") && strings.Contains(line, file) {
                        ...
                        fmt.Println(green(pkgName + " (instalado localmente)"))
                        foundLocal = true
                        break
                    }
                }
...
func findProvides(file string, searchRemote bool) {
    ...
    fmt.Printf("%s %s %s %s\n", cyan(">>>"), cyan("xbps-query"), cyan("-o"), yellow(searchPattern))
    cmdLoc := exec.Command("xbps-query", "-o", searchPattern)
    ...
    // 4. Camada xbps-query Remota (-Ro) - APENAS SE searchRemote FOR TRUE
    if searchRemote {
        fmt.Printf("%s %s %s %s\n", cyan(">>>"), cyan("xbps-query"), cyan("-Ro"), yellow(searchPattern))
        cmd := exec.Command("xbps-query", "-Ro", searchPattern)
        output, _ := cmd.Output()
        res := strings.TrimSpace(string(output))
        ...
        fmt.Printf("%s %s %s\n", yellow("[!]"), white("Busca remota ignorada."), cyan("Use -FR para busca profunda."))
    }
...
func cleanXbpsCache() {
    ...
    if os.Geteuid() != 0 {
        fmt.Printf("%s %s\n", yellow("[vinstall]"), white("A limpeza do cache requer root."))
    ...
    var pkgCount int
    var totalSize int64
    fmt.Printf("%s %s\n", cyan("[vinstall]"), white("Limpando cache em /var/cache/xbps..."))
    ...
    fmt.Printf("%s %s %s\n", yellow("[!]"), white("Liberado:"), green(formatBytes(totalSize)))

    cmd := exec.Command("xbps-query", "-O")
    out, _ := cmd.Output()
    if orphans := strings.TrimSpace(string(out)); orphans != "" {
        fmt.Printf("%s %s\n%s\n", yellow("[!]"), white("Órfãos:"), cyan(orphans))
...
func checkAndEnableService(pkgName string) {
    ...
    if info, err := os.Stat(servicePath); err == nil && info.IsDir() {
        if _, err := os.Lstat(targetPath); os.IsNotExist(err) {
            fmt.Printf("\n%s %s '%s'. Ativar? [s/N]: ", yellow("[!]"), white("Serviço para"), cyan(pkgName))
            reader := bufio.NewReader(os.Stdin)
...
func displaySearch(pkgs []Package, title string) {
    ...
    fmt.Printf("%s\n%s\n", cyan(title), white(strings.Repeat("─", width)))
    for i, p := range pkgs {
        ...
        fmt.Printf("%s %s %-30s %s\n", idx, status, white(p.FullName), green(p.Description))
    }
}
...
func displayMenu(pkgs []Package, flags []string) {
    ...
    displaySearch(pkgs, "\nSugestões:")
    fmt.Printf("%s", yellow("Opção ('q' p/ sair): "))
    reader := bufio.NewReader(os.Stdin)
    input, _ := reader.ReadString('\n')
...
func printUsage() {
    ...
    fmt.Printf("  %s %-15s %s\n", green("vinstall"), white("-F"), white("ifconfig (Busca local rápida)"))
    fmt.Printf("  %s %-15s %s\n", green("vinstall"), white("-FR"), white("ifconfig (Busca profunda remota)"))
    fmt.Println("\nAtalhos de Consulta:")
