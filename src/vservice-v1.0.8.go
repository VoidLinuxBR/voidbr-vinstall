/*
    ...
    Updated:   qua 24 jun 2026 13:00:00 -04
    Version:   1.0.8
...
func sendNotification(msg string) {
	url := os.Getenv("VSERVICE_WEBHOOK_URL")
	if url == "" { return }
	payload := []byte(fmt.Sprintf(`{"content": "vservice alert: %s"}`, msg))
	http.Post(url, "application/json", bytes.NewBuffer(payload))
}

func monitor() {
	info("Monitorando todos os serviços habilitados em " + ActiveDir + "...")

	// Lista todos os itens em /var/service
	services, err := os.ReadDir(ActiveDir)
	if err != nil {
		showErr("Erro ao ler diretório de serviços: " + err.Error())
		return
	}

	for _, s := range services {
		// Ignora diretórios que não são links (caso haja algo estranho lá)
		if s.Type()&os.ModeSymlink == 0 {
			continue
		}

		serviceName := s.Name()
		out, _ := exec.Command("sudo", "sv", "status", serviceName).Output()
		statusStr := strings.TrimSpace(string(out))

    if !strings.HasPrefix(statusStr, "run:") {
        alertMsg := fmt.Sprintf("Serviço %s está " + Red + "DOWN" + Reset + " ou falhou! Status: %s", serviceName, statusStr)
        showErr(alertMsg)
        sendNotification(alertMsg)
    } else {
      msg(fmt.Sprintf("Serviço %s está rodando (" + Green + "OK" + Reset + ")", serviceName))
    }
	}
}
...
func installCompletion() {
    ...
    local opts="enable add disable remove rm start st up stop down restart status list archive-logs monitor"
...
func showStatusNEW(s string) {
	if _, e := exec.LookPath("vsv"); e == nil {
		...
		for _, line := range lines {
			...
			// O vsv geralmente começa com o status (✔/✘)
			// Se a linha tem ícone, o nome é o segundo campo (index 1)
			// Se não tem, o nome é o primeiro campo (index 0)
			nameIdx := 0
			if fields[0] == "✔" || fields[0] == "✘" {
				nameIdx = 1
			}

			// Verifica se temos campos suficientes e se o nome é exatamente igual
			if len(fields) > nameIdx && fields[nameIdx] == s {
				fmt.Println(line)
			}
		}
	} else {
		// Mantive o padrão de chamada para sv status
