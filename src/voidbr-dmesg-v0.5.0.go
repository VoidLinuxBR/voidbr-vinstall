...
// colorize destaca o timestamp (se reconhecido) em ciano discreto e a
// ocorrência da query em amarelo negrito. lowerLine e lowerQuery já vêm
// pré-calculados pelo chamador para evitar refazer strings.ToLower()
// em cima da mesma linha/query repetidamente.
func colorize(line, lowerLine, query, lowerQuery string) string {
    ...
    case tsEnd == 0 && matchStart == -1:
        return line
    case tsEnd == 0:
        return line[:matchStart] + Bold + Yellow + line[matchStart:matchEnd] + Reset + line[matchEnd:]
    case matchStart == -1:
        return Timestamp + line[:tsEnd] + Reset + line[tsEnd:]
    case matchStart >= tsEnd:
        return Timestamp + line[:tsEnd] + Reset + line[tsEnd:matchStart] + Bold + Yellow + line[matchStart:matchEnd] + Reset + line[matchEnd:]
    default:
...
// scanAndWatch varre /var/log e passa a monitorar (via tailFile) qualquer
// arquivo elegível que ainda não esteja sendo observado. Chamado uma vez
// no início e depois periodicamente, para pegar logs criados depois que
// o monitor já estava rodando.
func scanAndWatch() {
    ...
    for {
        ...
        if err != nil {
            if err == io.EOF {
                ...
                if statErr != nil {
                    // Arquivo sumiu (removido) — mantém tentando, pode voltar
                    continue
                }
...
func main() {
    ...
    // Reescaneia /var/log periodicamente para pegar arquivos de log
    // criados depois que o monitor já estava rodando (ex.: serviço novo
