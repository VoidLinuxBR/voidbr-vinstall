/*
    ...
    Updated:   dom 05 jul 2026 11:35:00 -04
    Version:   0.3.4
...
func main() {
	...
	fmt.Println("--- Monitor Iniciado. [ESPAÇO]: Pausa | [DIGITE]: Novo Filtro + Enter ---")
	queryMu.RLock()
	fmt.Print(Yellow + "[FILTRO: " + dynamicQuery + "] " + Reset)
	queryMu.RUnlock()

	// Leitor unificado de teclado
	go func() {
		reader := bufio.NewReader(os.Stdin)
		for {
			// Lê um caractere ou linha
			input, _ := reader.ReadString('\n')
			input = strings.TrimSpace(input)

			if input == "" {
				// Foi um espaço ou enter vazio? Alterna pausa
				pauseMu.Lock()
				paused = !paused
				if paused {
					fmt.Print("\n" + Red + "[PAUSADO - Pressione Espaço/Enter para retomar] " + Reset)
				} else {
					fmt.Print("\r\033[K")
					queryMu.RLock()
					fmt.Print(Yellow + "[FILTRO: " + dynamicQuery + "] " + Reset)
