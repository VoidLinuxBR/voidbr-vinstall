...
func syncAndCheckUpdates() {
	...
	for _, p := range updates {
		if n := len(cleanVersion(p.FullName)); n > maxNameLen {
			maxNameLen = n
		}
	}
	if maxNameLen > 40 {
	...
	for i, p := range updates {
		...
		// Padding aplicado ANTES de colorir: %-*s conta bytes de escape ANSI
		// como parte do comprimento, então paddar depois de white() quebra
		// o alinhamento quando os nomes têm tamanhos diferentes.
		namePadded := fmt.Sprintf("%-*s", maxNameLen, cleanVersion(p.FullName))
		oldVer := extractVersion(p.Description)
		newVer := extractVersion(p.FullName)
		fmt.Fprintf(w, "%s %s %s → %s  (%s)\n", idx, white(namePadded), yellow(oldVer), green(newVer), magenta(formatBytes(p.SizeDownload)))
		totalDownload += p.SizeDownload
	}
	w.Flush()
