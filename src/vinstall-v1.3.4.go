/*
    ...
    Updated:   qua 04 fev 2026 10:10:00 -04
    Version:   1.3.4-20260204
...
// --- CORE: EXECUÇÃO (AJUSTADO PARA MÚLTIPLOS PACOTES) ---
...
func displayMenu(pkgs []Package, flags []string) {
	...
	fmt.Printf("\n%s\n%s\n", cyan("Sugestões encontradas no repositório:"), lineStr)
	for i, p := range pkgs {
...
// --- MAIN (FIX: SUPORTE A MÚLTIPLOS PACOTES) ---
...
func main() {
	...
	case "remove":
		if len(targets) > 0 { runBinary("xbps-remove", flags, targets) }
	default:
		if len(targets) > 0 {
			if !runBinary("xbps-install", flags, targets) {
				// Sugestões apenas para o primeiro termo se falhar
