/*
    ...
    Updated:   dom 05 jul 2026 09:30:00 -04
    Version:   0.3.2
...
func main() {
	...
	// Impressor Central com lógica de Pausa
	go func() {
	...
	fmt.Println("--- Monitor de Logs VoidBR Iniciado (Espaço para Pausar) ---")

	// Escuta de teclado para Filtro e Pausa
	go func() {
		scanner := bufio.NewScanner(os.Stdin)
		for scanner.Scan() {
			input := strings.TrimSpace(scanner.Text())
			if input == "" { // Espaço ou Enter vazio alterna pausa
				pauseMu.Lock()
				paused = !paused
				if paused {
