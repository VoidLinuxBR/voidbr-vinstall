/*
    ...
    Updated:   ter 03 fev 2026 15:15:10 -04
    Version:   1.0.8-20260203
...
func main() {
	...
	// Separa o que é flag (começa com -) do que é pacote
	...
	if len(targets) == 0 && len(flags) > 0 {
		// Caso o usuário use apenas 'vinstall -Su', por exemplo
		runXbps(flags, "")
		return
	}

	target := targets[0] // Focamos no primeiro pacote para a lógica de sugestão

	fmt.Printf("%s v%s\n", white("vinstall"), Version)
	fmt.Printf("%s %s %s...\n", cyan("[vinstall]"), white("📦 Verificando:"), yellow(target))

	// 1. Tenta a instalação com as flags passadas
