/*
   ...
   Changelog 0.6.1:
     - Corrige vazamento de file descriptor em tailFile: o defer
       capturava o *os.File original por valor no momento em que a
       goroutine iniciava, então qualquer tailFile que passasse por
       uma rotação de log nunca fechava o arquivo mais recente ao
       sair (só o antigo, já fechado manualmente). Agora o defer usa
       uma closure que resolve "file" no momento do retorno.
     - Corrige truncamento/preenchimento da barra de status por BYTE
       em vez de RUNE: com filtro contendo acentos (ç, ã, é...),
       len(msg) contava bytes UTF-8 em vez de caracteres, e
       msg[:cols] podia cortar um caractere multibyte ao meio,
       corrompendo a saída no terminal. Mesma classe de bug que o
       changelog 0.6.0 já tinha corrigido em inputLoop, mas que
       persistia aqui.
     - Corrige getTerminalSize(): sempre consultava syscall.Stdin via
       ioctl, mesmo quando quem interessa é o tamanho do STDOUT (a
       região de rolagem e a barra de status são desenhadas nele).
       Se stdin fosse redirecionado enquanto stdout continuava sendo
       um terminal real, o ioctl falhava e caía no fallback 24x80,
       desalinhando tudo. Agora recebe o fd explicitamente.
     - printStatus() agora respeita --no-color: antes aplicava os
       códigos ANSI da barra de status (StatusBar/Reset)
       incondicionalmente, mesmo com colorEnabled=false, deixando o
       conteúdo do log sem cor mas a barra ainda colorida.
...
var (
	...
	// colorEnabled controla se colorize() e a barra de status aplicam
	// códigos ANSI.
	colorEnabled bool
...
// associado a fd. Se falhar (ex.: fd não é um terminal, ou foi
// redirecionado para um arquivo), assume 24x80.
//
// Recebe o fd explicitamente porque quem importa aqui é o STDOUT (é
// nele que a região de rolagem e a barra de status são desenhadas) —
// consultar sempre syscall.Stdin faria o ioctl falhar (e cair no
// fallback 24x80) sempre que stdin estivesse redirecionado, mesmo com
// stdout sendo um terminal real de tamanho diferente.
func getTerminalSize(fd uintptr) (rows, cols int) {
	ws := &winsize{}
...
func setupTerminal() {
	rows, cols := getTerminalSize(os.Stdout.Fd())
	termMu.Lock()
...
func printStatus(buffer string) {
	...
	// Preenche/trunca até a largura do terminal contando RUNES, não
	// bytes: o filtro pode conter acentos (ç, ã, é...), e len(string)
	// em Go conta bytes UTF-8. Contar bytes faz o preenchimento sair
	// da largura real do terminal e, pior, msg[:cols] pode cortar um
	// caractere multibyte ao meio, corrompendo a saída.
	runes := []rune(msg)
	if len(runes) < cols {
		msg = string(runes) + strings.Repeat(" ", cols-len(runes))
	...
	if colorEnabled {
		fmt.Print(StatusBar + msg + Reset) // branco sobre azul
	} else {
		fmt.Print(msg) // --no-color: barra de status também sem ANSI
	}
...
func tailFile(p string) {
	...
	// IMPORTANTE: usa uma closure (não "defer file.Close()" direto) para
	// que "file" seja resolvida no momento em que o defer RODA (saída da
	// função), não no momento em que o defer é REGISTRADO. Como "file" é
	// reatribuída sempre que o arquivo é reaberto após uma rotação, um
	// "defer file.Close()" direto capturaria o *os.File original e, após
	// qualquer rotação, vazaria o file descriptor do arquivo mais recente
	// ao sair (só o antigo, já fechado manualmente no laço abaixo, seria
	// fechado de novo).
