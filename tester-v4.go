...
// Converte qualquer tipo numérico para int64 de forma segura
...
func main() {
	...
	for _, path := range repoPaths {
		...
		go func(p string) {
			...
			repoURL := repoMap[dirName]
			if repoURL == "" { repoURL = dirName }

			for _, pkgData := range index {
				pkg := pkgData.(map[string]interface{})
				if strings.Contains(strings.ToLower(fmt.Sprintf("%v", pkg["pkgver"])), query) {
					
					// Prioriza 'filename-size', se não achar, 'installed_size'
					sizeRaw, ok := pkg["filename-size"]
					if !ok {
						sizeRaw = pkg["installed_size"]
					}
	...
	if len(pkgs) == 0 {
		...
		for i, p := range pkgs {
			...
			fmt.Printf("%9s%s (%s)\n", "", cyan(p.Repo), yellow(formatBytes(p.Size)))
		}
		fmt.Println(lineSeparator)
	}
}
