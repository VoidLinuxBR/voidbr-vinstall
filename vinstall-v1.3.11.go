/*
    ...
    Updated:   ter 23 jun 2026 17:45:00 -04
    Version:   1.3.11
...
func getInstalledPackages() map[string]bool {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if len(fields) >= 2 { installed[fields[1]] = true }
	}
	return installed
}
...
func remoteSearchDetailed(query string) {
	...
	uniquePkgs := make(map[string]Package)
	var mu sync.Mutex
	...
	for dirName, repoURL := range repoMap {
		...
		go func(p, url string) {
			...
			for _, pkgData := range index {
				...
				fullText := fmt.Sprintf("%v %v %v %v", pkg["pkgver"], pkg["short_desc"], pkg["maintainer"], pkg["filename"])
				
				match := false
				if isRegex { match = re.MatchString(fullText) } else { match = strings.Contains(strings.ToLower(fullText), strings.ToLower(query)) }
	...
	var pkgs []Package
	for _, p := range uniquePkgs { pkgs = append(pkgs, p) }
	...
	if len(pkgs) == 0 {
		...
		for i, p := range pkgs { printPackage(writer, i, p, width) }
		writer.Flush()
		fmt.Println(white(strings.Repeat("─", width)))
	}
}

// --- FUNÇÕES DE BUSCA ORIGINAIS ---
...
func main() {
	...
	for _, arg := range args {
		...
		case "-F": mode = "find"
		case "-FR": mode = "find"; searchRemote = true
	...
	case "remote-search": if len(targets) > 0 { remoteSearchDetailed(targets[0]) }
	...
	default:
		if len(targets) > 0 {
			if !runBinary("xbps-install", flags, targets) {
				...
				if len(suggestions) > 0 { displayMenu(suggestions, flags) }
			} else { for _, t := range targets { checkAndEnableService(t) } }
		} else if len(flags) > 0 { runBinary("xbps-install", flags, []string{}) }
	}
}
...
func printUsage() {
	...
	fmt.Printf("  %s %-15s %s\n", green("vinstall"), white("-Ss"), white("\"regex\" (Busca Full-Text + Regex)"))
	fmt.Println("\nAtalhos de Consulta:")
	fmt.Printf("  %-20s %s\n", green("-Li"), white("Lista instalados"))
	fmt.Printf("  %-20s %s\n", green("-Lo"), white("Lista órfãos"))
	fmt.Printf("  %-20s %s\n", green("-Qs <query>"), white("Busca termo nos instalados"))
	fmt.Println()
}
