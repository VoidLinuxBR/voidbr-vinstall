...
const (
	...
	// execTimeout é o tempo máximo concedido a comandos externos (ex: xlocate,
	// xbps-query -R) antes de serem abortados, evitando que o processo trave.
...
func findProvides(file string, searchRemote bool) {
	...
	if xPath, err := exec.LookPath("xlocate"); err == nil {
		checkXlocateIndex()
		wg.Add(1)
		go func() {
			defer wg.Done()
...
func fetchSuggestions(query string) []Package {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if name != "" {
			pkgs = append(pkgs, Package{strings.TrimSpace(status), name, strings.TrimSpace(desc)})
		}
	}
	return pkgs
}
